 ...
 */
class WinnersTeamForm extends Model
{
    /**
    ...
    public function loadFromTournamentModel()
    {
        ...
        foreach ($tournament->tournamentPrizes as $prize) {
            if ($prize->type_id == TournamentPrize::TYPE_SPECIAL) {
                $winnerId = $tournament->getWinnerParticipantTeam(null, $prize->id, null);
                $this->prizes[$prize->id] = $winnerId;
            } elseif ($prize->type_id == TournamentPrize::TYPE_SECONDARY) {
                $winnerId = $tournament->getWinnerParticipantTeam(null, null, $prize->id);
                $this->prizes[$prize->id] = $winnerId;
            }
        }

        $this->player_one = $tournament->getWinnerParticipantTeam(1);
        $this->player_two = $tournament->getWinnerParticipantTeam(2);
        $this->player_three = $tournament->getWinnerParticipantTeam(3);
        $this->player_four = $tournament->getWinnerParticipantTeam(4);
    ...
    private function saveStandardPrizes()
    {
        ...
        if ($this->player_one) {
            $this->rewardService->removeRewardForTeams($tournament->id, 1, null);
            $this->rewardService->addRewardForTeam($tournament->id, $this->player_one, 1, null);
        } else {
            $this->rewardService->removeRewardForTeams($tournament->id, 1, null);
        }

        if ($this->player_two) {
            $this->rewardService->removeRewardForTeams($tournament->id, 2, null);
            $this->rewardService->addRewardForTeam($tournament->id, $this->player_two, 2, null);
        } else {
            $this->rewardService->removeRewardForTeams($tournament->id, 2, null);
        }

        if ($this->player_three) {
            $this->rewardService->removeRewardForTeams($tournament->id, 3, null);
            $this->rewardService->addRewardForTeam($tournament->id, $this->player_three, 3, null);
        } else {
            $this->rewardService->removeRewardForTeams($tournament->id, 3, null);
        }

        if ($this->player_four) {
            $this->rewardService->removeRewardForTeams($tournament->id, 4, null);
            $this->rewardService->addRewardForTeam($tournament->id, $this->player_four, 4, null);
        } else {
            $this->rewardService->removeRewardForTeams($tournament->id, 4, null);
        }
    ...
    private function saveDynamicPrizes()
    {
        ...
        foreach ($this->prizes as $prizeId => $playerId) {
            ...
            if ($prize->type_id == TournamentPrize::TYPE_SPECIAL) {
                $this->rewardService->removeRewardForTeams($tournament->id, null, $prizeId, null);
                if ($playerId) {
                    $this->rewardService->addRewardForTeam($tournament->id,
                        $playerId, null, $prizeId, null);
                }
            } elseif ($prize->type_id == TournamentPrize::TYPE_SECONDARY) {
                $this->rewardService->removeRewardForTeams($tournament->id, null, null, $prizeId);
                if ($playerId) {
                    $this->rewardService->addRewardForTeam($tournament->id,
    ...
    // {
    //     return $this->getTournament()->getParticipantsTeams($onlyNames);
    // }
