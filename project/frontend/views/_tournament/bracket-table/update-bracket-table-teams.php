...
/** @var $this View */
/** @var $model BracketTableTeamsForm */

$bracketTableRows = $model->bracketTableRowsTeam;
$bracketParticipants = $model->bracketParticipants;

$rowsParticipantsIds = [];
foreach ($bracketTableRows as $bracketTableRow) {
    $rowsParticipantsIds[$bracketTableRow->tournament_to_team_id] = $bracketTableRow->id;
}
?>

<div class="popup" id="adminBracketsInsertParticipants2">
    <div class="popup-wrap">
        <div class="popup-main">
            <?php Pjax::begin([
                'id' => 'bracketTableTeamsForm',
                'enablePushState' => false,
                'enableReplaceState' => false,
            ]); ?>
            <?= Html::beginForm(['/bracket-table/update-bracket-table-teams', 'id' => $model->tournament_id, 'bracketId' => $model->id], 'post', [
                'data-pjax' => 1,
                            ...
                            <div class="table-inner">
                                ...
                                <?php else : ?>
                                    <table>
                                        ...
                                        <?php
                                        foreach ($bracketParticipants as $participantId => $participant) :
                                        ...
                                        <tr <?= $isAttached ? '' : 'class="disabled"' ?> data-checkbox>
                                                ...
                                                <div class="checkbox checkbox--toggler">
                                                    <label class="checkbox-label" for="insertParticipants<?= $participantId ?>">
                                                        <input class="checkbox-input" type="checkbox" id="insertParticipants<?= $participantId ?>"
                                                            name="<?= 'BracketTableRowTeam[' . $participantId . '][active]' ?>"
                                            ...
                                            <td>
                                                <?= Html::hiddenInput('BracketTableRowTeam[' . $participantId . '][id]', $rowsParticipantsIds[$participantId] ?? null , []) ?>
                                                <?= Html::hiddenInput('BracketTableRowTeam[' . $participantId . '][tournament_to_team_id]', $participantId, []) ?>
...
$js = <<<JS
    ...
    $(document).on('change', '#insertParticipantsAll', function() {
        $('#bracketTableTeamsForm [data-checkbox] input[type=checkbox]').prop('checked', $(this).prop('checked')).change();
    });
