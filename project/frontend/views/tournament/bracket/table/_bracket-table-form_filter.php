        ...
        <div class="filter-wrap">
            <div class="filter-inner">
                ...
                <div class="filter-btns">
                    ...
                    <div class="filter-btn">
                        <button type="button" class="btn btn--sm js-ajax-popup" data-pjax="0"
                            data-url="<?= Url::to(['/bracket-table/update-bracket-table-participants', 'id' => $bracket->tournament_id, 'bracketId' => $bracket->id]) ?>">
                            <?= $bracket->bracketTableRows ? 'Edit' : 'Insert' ?> participants
                        </button>
                    </div>
                </div>
...
$js = <<<JS
    $(document).on('keyup', '.js-table-search', function(event) {
        ...
        $('.filter-content tbody tr').each(function() {
            let thisRow = $(this); 
            if (thisRow.find('.table-player__name').text().search(new RegExp(searchValue, "i")) > -1) {
                thisRow.show();
            } else {
                thisRow.hide();
            }
